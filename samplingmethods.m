...
while(1)         %an infinit while loop to loop agaain if the user wishes to continue
    ...
    if ReplyMode == 1       %if the user put 1 then we sttart the wave analysis mode
        ...
        ReplySamMeth=input('\nwhich sampling method do you want 2 for 2 sample method ,3 for 3 sample method\n,P for peak-Based predictive calculations, D for Discrete fourier transform and 1 for all ;) : ans: ','s');
        ...
        if ReplySamMeth=='1' || ReplySamMeth=='2' || ReplySamMeth=='3' || ReplySamMeth=='P' ||ReplySamMeth=='D'    %fail safe as if the answer is not elligible default to one
        else
            ReplySamMeth='1';
        ...
        if ReplySamMeth=='D' || ReplySamMeth=='1'  
            ...
            Yfmean=mean(Yfinal);
            thetafinal=transpose(ThetaDFT);
            thetafmean=mean(thetafinal);
            figure('Name','magnitude over frequency for DFT','NumberTitle','off');  % plotting the magnitude wave(here I name the figure and remove the number title requirment in its entirity)
            ...
            writematrix(MD4,'Output\dft.xlsx') %here I write into the excel files             
        end
        if ReplySamMeth=='2' || ReplySamMeth=='1'   %if TwoSample method is to be used
            ...
            figure('Name','magnitude over time for two sample method','NumberTitle','off');  % plotting the magnitude wave(here I name the figure and remove the number title requirment in its entirity)
            plot(time,Y,'-bo');                                                         
            title('magnitude over time for two sample method')                            
            figure('Name','angle over time for two sample method','NumberTitle','off'); 
            plot(time,angle,'-ko');                                                      
...
%version 1.61 (the file Mangment update)
%minor bug fixes
